 ...
 * This was inspired by the cafe example given by El in class.
 */
...
app.post("/checkout", async (req, res, next) => {
    try {
        ...
        // Read data from both potions.json and custom-potion.json
        const [potionsData, customPotionsData] = await Promise.all([
            fs.readFile("potions.json", "utf8"),
            fs.readFile("custom-potion.json", "utf8")
        ]);
        const potions = JSON.parse(potionsData); // Regular potions
        const customPotions = JSON.parse(customPotionsData); // Custom potions

        let totalCost = 0; // To calculate the total cost of the order
        let unavailableItems = []; // To track items that are out of stock or not found

        // Validate each item in the cart
        for (const item of cart) {
            const potion = potions.find(p => p.name === item.name); // Check in regular potions
            const customPotion = customPotions.find(cp => cp.name === item.name); // Check in custom potions

            if (potion) {
                // If it's a regular potion, check stock
                if (!potion.in_stock) {
                    unavailableItems.push({ name: item.name, reason: "Out of stock." });
                } else {
                    totalCost += potion.price * item.quantity; // Add to total cost
                }
            } else if (customPotion) {
                // If it's a custom potion, add to total cost (custom potions don't have stock constraints)
                totalCost += customPotion.price * item.quantity;
            } else {
                // Potion not found in either dataset
